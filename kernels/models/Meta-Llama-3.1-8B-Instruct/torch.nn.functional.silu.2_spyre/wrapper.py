...
that op's ``triton_kernel.py`` docstring for the full breakdown), but traced
on a logical ``f16[1, 1, 14336]`` input here (batch dim of size 1, vs. 12
for ``silu.1``) -- so every descriptor is 2-D (``shape=[224, 64]``) instead
of 3-D, and there is no leading ``c0 = 0`` / ``dim0`` boilerplate (Inductor
folds the size-1 batch dim away entirely for this trace).

Each of the 5 stages is exposed as its own function below (``silu_clone``,
``silu_neg``, ``silu_exp``, ``silu_add_one``, ``silu_div``), plus a
composite ``silu()`` that chains all 5 in the order the original
``Runner.call`` launches them, with a per-stage ``SIGNATURE`` override where
its pointer args differ from the module-level ``SIGNATURE``.

Caller-side settings, common to all 5 stages, from the traced
``output_code.py``:

- ``assert_size_stride(arg0_1, (1, 1, 14336), (14336, 14336, 1))`` —
  logical input, dtype ``torch.float16``.
- Every ``.run(...)`` call passes ``xnumel=14336``.
- ``config={'XBLOCK': 448}``, ``triton_meta={..., 'spyre_grid': (32,)}`` —
...
def silu_clone(in_ptr0: torch.Tensor, kernel_fn=triton_kernel.triton_unk_fused_0) -> torch.Tensor:
    ...
    stage clones the logical `f16[1, 1, 14336]` input verbatim; the
...
def silu_div(in_out_ptr0: torch.Tensor, in_ptr0: torch.Tensor, kernel_fn=triton_kernel.triton_unk_fused_silu_4) -> torch.Tensor:
    ...
    its role, same as `torch.nn.functional.silu.1_spyre`."""
...
def make_inputs_neg(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    """Stage 2 (``neg``): ``in_ptr0`` -> ``out_ptr0``, both ``[224, 64]``."""
    del xnumel, XBLOCK
    in_ptr0 = _rand_buf(1)
    out_ptr0 = np.zeros((224, 64), dtype=np.float16)
    return {"in_ptr0": in_ptr0, "out_ptr0": out_ptr0}
...
def make_inputs_exp(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    """Stage 3 (``exp``): in-place on ``in_out_ptr0``, ``[224, 64]``."""
    del xnumel, XBLOCK
...
def make_inputs_add_one(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    """Stage 4 (``+1``): in-place on ``in_out_ptr0``, ``[224, 64]``."""
    del xnumel, XBLOCK
...
def make_inputs_div(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    ...
    as the numerator source; both ``[224, 64]``."""
    del xnumel, XBLOCK
