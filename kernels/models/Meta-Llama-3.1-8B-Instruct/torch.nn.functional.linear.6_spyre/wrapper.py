...
``wrapper.py`` docstring for the full field-by-field rationale). M=1 (a
...
# Compiles cleanly through Triton's frontend to TTIR (every constexpr
# tile size in both bundled sub-kernels happens to be a power of 2
# here), but the ConvertFunctions pass (third_party/spyre/lib/Dialect/
# KTDP/Transforms/ConvertFunctions.cpp) has no rewrite pattern for the
# tt.call ops that invoke triton_bundle_0's noinline=True helpers, so
# MLIR verification rejects the module once ConvertFunctions has
# converted the callees to func::FuncOp. This is a genuine
# backend/pipeline gap (confirmed against the production inliner too,
# which correctly leaves noinline calls un-inlined), not an extraction
...
def linear(in_ptr0: torch.Tensor, in_ptr1: torch.Tensor, kernel_fn=triton_kernel.triton_bundle_0) -> torch.Tensor:
    """nn.functional.linear (no bias), M=1 (decode step), K=4096, N=4096.

    KNOWN BROKEN: see DISABLED_REASON above -- calling this reproduces the
    traced source's genuine KTIR-lowering-pipeline gap.
    """
