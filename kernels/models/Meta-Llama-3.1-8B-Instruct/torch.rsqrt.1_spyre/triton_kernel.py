"""``torch.rsqrt.1_spyre`` kernel extracted from a torch-spyre Inductor trace.
...
Meta-Llama-3.1-8B-Instruct's traced ``torch.rsqrt.1`` op
(``aten.rsqrt.default(arg0_1)`` — unary — on a logical ``f16[1, 12, 1]``
tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.rsqrt.1_spyre/
torch_compile_debug/run_2026_07_14_12_21_06_361826-pid_257867/
...
@triton.jit
def triton_unk_fused_rsqrt_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    xnumel = 12
    ...
    tmp1 = tl.rsqrt(tmp0.to(tl.float32)).to(tl.float16)
