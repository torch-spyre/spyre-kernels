...
# read by the real descriptor-driven computation, see
# triton_kernel.py's docstring), but Triton's frontend still eagerly
# compiles `tl.arange(0, XBLOCK)` and requires a power-of-2 range.
# 12 isn't one, so it's rounded up to 16 here — this has zero effect
# on the kernel's actual output since xindex/xmask are never used.
XBLOCK = 16
GRID = (32,)
...
def matmul(
    ...
    """Broadcast elementwise multiply implementing the degenerate bmm:
    each of `in_ptr0`'s 64 padded rows is multiplied by `in_ptr1`'s single
    padded stick. Output shape `[1, 1, 64, 64]` differs from `in_ptr0`'s
    `[64, 1, 1, 64]`, so it is allocated directly rather than via
    `empty_like`.
    """
    out_ptr0 = torch.empty((1, 1, 64, 64), dtype=in_ptr0.dtype, device=in_ptr0.device)
    kernel_fn[GRID](in_ptr0, in_ptr1, out_ptr0, YNUMEL, XNUMEL, YBLOCK=YBLOCK, XBLOCK=XBLOCK)
    return out_ptr0
...
def make_inputs(
    xnumel: int = XNUMEL, ynumel: int = YNUMEL, XBLOCK: int = XBLOCK, YBLOCK: int = YBLOCK,
    **_unused,
) -> dict:
    ...
    ``triton_kernel.py``): ``in_ptr0`` is ``[64, 1, 1, 64]``, ``in_ptr1`` is
