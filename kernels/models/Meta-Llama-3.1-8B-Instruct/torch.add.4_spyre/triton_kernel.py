"""``torch.add.4_spyre`` kernel extracted from a torch-spyre Inductor trace.
...
Meta-Llama-3.1-8B-Instruct's traced ``torch.add.4`` op
(``aten.add.Tensor(arg0_1, arg1_1)`` — tensor+tensor, both f16 — on a logical
``f16[1, 12, 4096]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.add.4_spyre/
torch_compile_debug/run_2026_07_14_12_45_11_957619-pid_265450/
...
def triton_unk_fused_add_0(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    desc_1 = tl.make_tensor_descriptor(in_ptr1, shape=[12, 64, 1, 64], strides=[4096, 64, 64, 1], block_shape=[12, 2, 1, 64])
    desc_2 = tl.make_tensor_descriptor(out_ptr0, shape=[12, 64, 1, 64], strides=[4096, 64, 64, 1], block_shape=[12, 2, 1, 64])
