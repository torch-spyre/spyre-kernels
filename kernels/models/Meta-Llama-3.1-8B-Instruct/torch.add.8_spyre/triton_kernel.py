"""``torch.add.8_spyre`` kernel extracted from a torch-spyre Inductor trace.

The ``@triton.jit`` function body (lines 76-94 of the ``async_compile.triton(...)``
source string) verbatim, from torch-spyre's Inductor output for
Meta-Llama-3.1-8B-Instruct's traced ``torch.add.8`` op
(``aten.add.Tensor(arg0_1, arg1_1)`` — tensor+tensor, both f16 — on a logical
``f16[1, 1, 4096]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.add.8_spyre/
torch_compile_debug/run_2026_07_14_13_30_45_017381-pid_275364/
...
def triton_unk_fused_add_0(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    desc_1 = tl.make_tensor_descriptor(in_ptr1, shape=[1, 64, 1, 64], strides=[4096, 64, 64, 1], block_shape=[1, 2, 1, 64])
