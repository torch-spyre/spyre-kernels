"""``torch.nn.functional.linear.2_spyre`` kernel extracted from a
...
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.nn.functional.linear.2``
op (``aten.mm`` between a logical ``f16[1, 12, 4096]`` activation and a
logical ``f16[1024, 4096]`` weight, i.e. `nn.functional.linear` without
bias, out_features=1024); see ``torch-spyre/test_results_triton_20260714_120532/
torch.nn.functional.linear.2_spyre/torch_compile_debug/
run_2026_07_14_12_25_58_795831-pid_259571/torchinductor/
model__0_inference_0.0/output_code.py``.

Bundled multi-kernel trace, same structure as ``torch.nn.functional.linear.1
_spyre`` (see that folder's ``triton_kernel.py`` docstring for the full rationale):
...
``triton_bundle_0`` is the entry that calls both in sequence. Only
difference from ``.1_spyre`` is the weight's `out_features` (N) dimension:
1024 here vs. 4096 there, which reshapes the tile counts/grid but not the
kernel structure.
...
def triton_bundle_0_kernel_1(in_ptr0, in_ptr1, out_ptr0, ynumel, xnumel, r0_numel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    dim2 = c2 // 64
    dim3 = c2 % 64
    desc_0 = tl.make_tensor_descriptor(in_ptr0, shape=[12, 1, 64, 64], strides=[4096, 64, 64, 1], block_shape=[6, 1, 64, 64])
    # Logical layouts -> Device layouts
    dim_1_0 = c2
    dim_1_1 = c1 // 64
    dim_1_2 = c1 % 64
    desc_1 = tl.make_tensor_descriptor(in_ptr1, shape=[4096, 16, 64], strides=[64, 262144, 1], block_shape=[4096, 1, 64])
    # Logical layouts -> Device layouts
    dim_2_0 = c0
    dim_2_1 = 0
    dim_2_2 = c1 // 64
    dim_2_3 = c1 % 64
    desc_2 = tl.make_tensor_descriptor(out_ptr0, shape=[12, 1, 16, 64], strides=[1024, 64, 64, 1], block_shape=[6, 1, 1, 64])
    ...
    tmp4 = tl.reshape(tmp1, [6, 1, 4096])
    tmp5 = tl.reshape(tmp3, [4096, 64])
    tmp6 = tl.reshape(tmp4, [6, 4096])
    tmp7 = tl.dot(tmp6, tmp5, input_precision="ieee")
    tmp8 = tmp7.to(tl.float16)
    tmp9 = tl.reshape(tmp8, [6, 1, 1, 64])
...
def triton_bundle_0(in_ptr0, in_ptr1, out_ptr0, out_ptr1):
    ...
    triton_bundle_0_kernel_1(in_ptr1, out_ptr0, out_ptr1, 12, 1024, 4096, 6, 64)
