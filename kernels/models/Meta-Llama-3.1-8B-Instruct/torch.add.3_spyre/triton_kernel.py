"""``torch.add.3_spyre`` kernel extracted from a torch-spyre Inductor trace.
...
Meta-Llama-3.1-8B-Instruct's traced ``torch.add.3`` op
(``aten.add.Tensor(arg0_1, arg1_1)`` — tensor+tensor, both f16 — on a logical
``f16[1, 8, 12, 128]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.add.3_spyre/
torch_compile_debug/run_2026_07_14_12_36_59_027039-pid_262945/
...
def triton_unk_fused_add_0(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    desc_1 = tl.make_tensor_descriptor(in_ptr1, shape=[8, 12, 2, 1, 64], strides=[1536, 128, 64, 64, 1], block_shape=[4, 1, 2, 1, 64])
    desc_2 = tl.make_tensor_descriptor(out_ptr0, shape=[8, 12, 2, 1, 64], strides=[1536, 128, 64, 64, 1], block_shape=[4, 1, 2, 1, 64])
