...
def zeros(
    ...
    kernel_fn=triton_kernel.triton_unk_fused_zeros_0,
) -> torch.Tensor:
    """Zero-fill implementing `aten.full.default(..., 0, dtype=...)`: loads
    the zero-valued seed stick `in_ptr0` and broadcasts/stores it across the
    full `[1, 2048, 128, 64]` output tile. Output shape differs from
    `in_ptr0`'s `[1, 1, 1, 64]`, so `torch.empty` (not `empty_like`) is used.
    """
    out_ptr0 = torch.empty((1, 2048, 128, 64), dtype=in_ptr0.dtype, device=in_ptr0.device)
...
def make_inputs(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    ...
    directly from ``tl.program_id(0)``, see ``triton_kernel.py``): ``in_ptr0``
    (the zero seed) is built at its full descriptor ``shape`` ``[1, 1, 1,
    64]`` (all zeros, matching the traced ``spyre_constant_tensor(0.0,
    ...)``), and ``out_ptr0`` at its full descriptor ``shape`` ``[1, 2048,
    128, 64]``.
    """
    del xnumel, XBLOCK
