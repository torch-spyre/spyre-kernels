"""``torch.nn.functional.linear.8_spyre`` kernel extracted from a
torch-spyre Inductor trace.

The three ``@triton.jit`` functions verbatim from torch-spyre's Inductor
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.nn.functional.linear.8``
op — ``aten.bmm`` between a logical ``f16[1, 1, 4096]`` activation
(unsqueezed/expanded) and a logical ``f16[1, 4096, 4096]`` weight
...
torch.nn.functional.linear.8_spyre/torch_compile_debug/
run_2026_07_14_13_29_57_088965-pid_275131/torchinductor/
...
- ``triton_bundle_0_kernel_0`` — repacks the raw weight tensor
  (``arg1_1``, logical shape ``[4096, 4096]``) into a device-friendly
...
- ``triton_bundle_0_kernel_1`` — the actual matmul: loads a tile of the
  ...
  Unlike ``linear.7``'s ``kernel_1``, this one's activation descriptor
  ``block_shape`` (``[1, 1, 64, 64]``) is fully equal to its own ``shape``
  (``[1, 1, 64, 64]``) — i.e. no K-axis truncation: the single ``tl.dot``
  here contracts over the *entire* ``K=4096`` (``tmp6`` reshaped to
  ``[1, 4096]``, matching ``tmp5`` reshaped to ``[4096, 128]``).
