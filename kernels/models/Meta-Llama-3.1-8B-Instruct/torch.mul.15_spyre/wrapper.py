...
Structurally identical to ``torch.mul.14_spyre`` — same shapes, strides,
and kernel body, just a distinct traced call site (see ``triton_kernel.py``
for the exact source path). See ``torch.mul.14_spyre/wrapper.py`` for the
...
- logical ``x``: ``[1, 8, 1, 128]`` f16; logical ``y``: ``[1, 1, 1, 128]``
  f16, broadcast over ``x``'s head dim; logical output: same shape as ``x``
  but a different physical device layout (head axis moved front-to-back).
- device shapes: ``in_ptr0`` ``[8, 1, 2, 1, 64]``, ``in_ptr1`` ``[1, 1, 2,
  1, 64]``, ``out_ptr0`` ``[1, 1, 2, 8, 64]``.
- ``xnumel = 1024``, ``spyre_grid = (16,)``, one program per (head, stick)
  pair (8 heads x 2 sticks).
"""
...
SUMMARY = (
    ...
    "traced `torch.mul.15` op (structurally identical to "
    "`torch.mul.14`): `y` broadcast over `x`'s 8-wide head axis, "
    "one program per (head, stick) pair."
)
...
DOC = (
    ...
    "`torch.mul.14_spyre` (see that example's `doc` for the full "
    "device-layout explanation, including the note that `xmask` "
    "is a live-but-still-dead expression here, unlike most of the "
    "mul.N family), just a distinct traced call site."
)
...
DISTRIBUTION_LOOP = False  # see torch.mul.14_spyre/wrapper.py
...
def mul(
    ...
    `[1, 1, 2, 8, 64]` (head axis moved front-to-back). See
    `torch.mul.14_spyre/wrapper.py` for the full derivation."""
...
def make_inputs(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    ...
    ``torch.mul.14_spyre/wrapper.py`` for the full rationale).
    """
    del xnumel, XBLOCK
...
def run(inputs: dict) -> np.ndarray:
    """NumPy oracle: identical derivation to ``torch.mul.14_spyre/wrapper.py``.
    """
