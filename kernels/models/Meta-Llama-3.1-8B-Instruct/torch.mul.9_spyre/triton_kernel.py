"""``torch.mul.9_spyre`` kernel extracted from a torch-spyre Inductor trace.

The ``@triton.jit`` function body (from the ``async_compile.triton(...)``
source string) verbatim, from torch-spyre's Inductor output for
Meta-Llama-3.1-8B-Instruct's traced ``torch.mul.9`` op
(``aten.mul.Tensor(x, 1.0)`` on a logical ``f16[1, 1, 128]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.mul.9_spyre/
torch_compile_debug/run_2026_07_14_12_57_21_029299-pid_268354/
...
def triton_unk_fused_mul_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    c0 = (tl.program_id(0)) * 64
    # Logical layouts -> Device layouts
    dim0 = 0
    dim1 = c0 // 64
    dim2 = 0
    dim3 = c0 % 64
    desc_0 = tl.make_tensor_descriptor(in_ptr0, shape=[1, 2, 1, 64], strides=[128, 64, 64, 1], block_shape=[1, 1, 1, 64])
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[1, 2, 1, 64], strides=[128, 64, 64, 1], block_shape=[1, 1, 1, 64])
