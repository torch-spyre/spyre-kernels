"""``torch.nn.functional.linear.9_spyre`` kernel extracted from a
torch-spyre Inductor trace.

The three ``@triton.jit`` functions verbatim from torch-spyre's Inductor
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.nn.functional.linear.9``
op — ``aten.bmm`` between a logical ``f16[1, 1, 4096]`` activation
(unsqueezed/expanded) and a logical ``f16[1, 4096, 14336]`` weight
...
torch.nn.functional.linear.9_spyre/torch_compile_debug/
run_2026_07_14_13_31_46_969266-pid_275634/torchinductor/
...
- ``triton_bundle_0_kernel_0`` — repacks the raw weight tensor
  (``arg1_1``, logical shape ``[14336, 4096]``) into a device-friendly
  tiled layout (via ``tl.reshape``/``tl.permute``/``tl.reshape``).
- ``triton_bundle_0_kernel_1`` — the actual matmul: loads a tile of the
  ...
  ="ieee")``, casts the result back to ``float16``, and stores it. As in
  ``linear.8``, this ``kernel_1``'s activation descriptor ``block_shape``
  (``[1, 1, 64, 64]``) is fully equal to its own ``shape`` — no K-axis
  truncation: the single ``tl.dot`` contracts over the *entire*
  ``K=4096`` (``tmp6`` reshaped to ``[1, 4096]``, matching ``tmp5``
  reshaped to ``[4096, 448]``).
...
def triton_bundle_0_kernel_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    dim1 = c0
    dim2 = c1 % 64
    desc_0 = tl.make_tensor_descriptor(in_ptr0, shape=[64, 14336, 64], strides=[917504, 64, 1], block_shape=[64, 448, 64])
    # Logical layouts -> Device layouts
    dim_1_0 = c0 // 64
    dim_1_1 = c1
    dim_1_2 = c0 % 64
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[224, 4096, 64], strides=[262144, 64, 1], block_shape=[7, 4096, 64])
    tmp0 = desc_0.load([dim0, dim1, dim2])
    tmp1 = tl.reshape(tmp0, [64, 7, 64, 64])
    tmp2 = tl.permute(tmp1, [1, 0, 3, 2])
    tmp3 = tl.reshape(tmp2, [7, 4096, 64])
...
def triton_bundle_0_kernel_1(in_ptr0, in_ptr1, out_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr):
    ...
    dim_1_1 = c0 // 64
    dim_1_2 = c0 % 64
    desc_1 = tl.make_tensor_descriptor(in_ptr1, shape=[4096, 224, 64], strides=[64, 262144, 1], block_shape=[4096, 7, 64])
    # Logical layouts -> Device layouts
    dim_2_0 = 0
    dim_2_1 = 0
    dim_2_2 = c0 // 64
    dim_2_3 = c0 % 64
    desc_2 = tl.make_tensor_descriptor(out_ptr0, shape=[1, 1, 224, 64], strides=[14336, 64, 64, 1], block_shape=[1, 1, 7, 64])
...
def triton_bundle_0(in_ptr0, in_ptr1, out_ptr0, out_ptr1):
    ...
    triton_bundle_0_kernel_1(in_ptr1, out_ptr0, out_ptr1, 14336, 4096, 448)
