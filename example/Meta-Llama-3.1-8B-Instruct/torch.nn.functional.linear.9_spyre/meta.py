...
- ``assert_size_stride(arg1_1, (14336, 4096), (4096, 1))`` — logical
...
- ``in_ptr0`` (weight, consumed by ``kernel_0``'s ``desc_0``): physical
  shape ``[64, 14336, 64]`` (``64*14336*64 == 14336*4096``), a plain
  contiguous reshape of the logical ``(14336, 4096)`` weight.
...
- ``out_ptr0`` (scratch, written by ``kernel_0``'s ``desc_1``): physical
  shape ``[224, 4096, 64]`` (``224*4096*64 == 58720256``). Only needs to
  be large enough; initial content is irrelevant since ``kernel_0``
...
- ``out_ptr1`` (final output, ``kernel_1``'s ``desc_2``): physical shape
  ...
  of the logical ``(1, 1, 14336)`` output.

As in ``linear.8``, ``kernel_1``'s activation descriptor here
(``desc_0``, ``block_shape=[1, 1, 64, 64]``) is fully equal to its own
``shape`` -- no K-axis truncation. The single ``tl.dot`` genuinely
contracts over the entire ``K=4096`` (``tmp6`` reshaped to ``[1, 4096]``
against ``tmp5`` reshaped to ``[4096, 448]``), so the oracle below (full
...
``7`` itself traces back to this op's ``N=14336 == 224*64`` with
...
def make_inputs(**_unused) -> dict:
    ...
    weight = rng.standard_normal((14336, 4096)).astype(np.float16)
    x = rng.standard_normal((1, 1, 4096)).astype(np.float16)
    in_ptr0 = weight.reshape(64, 14336, 64)
    in_ptr1 = x.reshape(1, 1, 64, 64)
    out_ptr0 = np.zeros((224, 4096, 64), dtype=np.float16)
    out_ptr1 = np.zeros((1, 1, 224, 64), dtype=np.float16)
    return {
        "in_ptr0": in_ptr0,
        "in_ptr1": in_ptr1,
        "out_ptr0": out_ptr0,
...
def run(inputs: dict) -> np.ndarray:
    ...
    weight = inputs["in_ptr0"].reshape(14336, 4096).astype(np.float32)
    out = x @ weight.T  # (1, 1, 14336)
    return out.reshape(1, 1, 224, 64).astype(np.float16)
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "traced `torch.nn.functional.linear.9` op: x[1,1,4096] @ "
            "weight[14336,4096].T -> out[1,1,14336], as a bundle of a "
        ...
        "doc": (
            ...
            "programs, one per 448-wide output-feature tile) to load an "
        ...
        # same category of bug as linear.1-.5_spyre, just discovered one
        # stage later here (this XBLOCK feeds a dead tl.arange that
        # Triton's frontend rejects at plain compile_to_ttir time, before
        # any Spyre-specific lowering runs). Not fixable via meta.py: the
        # compiled entry (triton_bundle_0) exposes no constexpr.
        "disabled": {
            "reason": (
                ...
                "this op's N=14336 == 224*64 == 32*7*64), and Triton's own "
