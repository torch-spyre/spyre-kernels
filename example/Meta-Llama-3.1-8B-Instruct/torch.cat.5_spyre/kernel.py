"""``torch.cat.5_spyre`` kernel extracted from a torch-spyre Inductor trace.

The three ``@triton.jit`` functions verbatim (lines 61-118 of the
``async_compile.triton(...)`` source string), from torch-spyre's Inductor
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.cat.5`` op
(``aten.cat.default([x, y], dim=-1)`` on two logical ``f16[1, 32, 1, 64]``
tensors, producing a logical ``f16[1, 32, 1, 128]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.cat.5_spyre/
torch_compile_debug/run_2026_07_14_13_15_43_708796-pid_272170/
...
One program per row of the outermost (size-32) dim: ``c0 =
tl.program_id(0)`` indexes ``dim0`` directly, 32 programs total.
...
def triton_bundle_0_kernel_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    dim3 = 0
    dim4 = c1
    desc_0 = tl.make_tensor_descriptor(in_ptr0, shape=[32, 1, 1, 1, 64], strides=[64, 64, 64, 64, 1], block_shape=[1, 1, 1, 1, 64])
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[32, 1, 2, 1, 64], strides=[128, 128, 64, 64, 1], block_shape=[1, 1, 2, 1, 64])
...
def triton_bundle_0_kernel_1(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[32, 1, 2, 1, 64], strides=[128, 128, 64, 64, 1], block_shape=[1, 1, 2, 1, 64])
    tmp0 = desc_0.load([dim0, dim1, dim2, dim3, dim4])
    tmp1 = tl.reshape(tmp0, [1, 1, 1, 1, 64])
    tmp2 = tl.broadcast_to(tmp1, [1, 1, 2, 1, 64])
    desc_1.store([dim_1_0, dim_1_1, dim_1_2, dim_1_3, dim_1_4], tmp2)

@triton.jit
def triton_bundle_0(in_ptr0, in_ptr1, out_ptr0):
    triton_bundle_0_kernel_0(in_ptr0, out_ptr0, 2048, 64)
    triton_bundle_0_kernel_1(in_ptr1, out_ptr0, 2048, 64)
