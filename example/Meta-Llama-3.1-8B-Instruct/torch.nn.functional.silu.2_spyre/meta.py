...
Same 5-stage decomposition and harness treatment as
``torch.nn.functional.silu.1_spyre`` (see that op's ``meta.py`` docstring
for the ``SIGNATURE`` override rationale) -- traced on a logical
``f16[1, 1, 14336]`` input here, so every descriptor is the 2-D
``[224, 64]`` device layout directly (no leading batch dim), unlike
``silu.1``'s ``[12, 224, 64]``. See ``kernel.py``'s docstring for the exact
traced source path.
...
_COMMON = {
    "constexpr":  ["XBLOCK"],
    "params":     {"xnumel": [14336], "XBLOCK": [448]},
    "grid":       [32],
...
VARIANTS = {
    "default": {
        **_COMMON,
        ...
        "summary": (
            "Stage 1/5 of `torch.nn.functional.silu.2_spyre`: `aten.clone` "
        ...
        "doc": (
            ...
            "`f16[1, 1, 14336]` input verbatim; the clone is read again by "
    ...
    "neg": {
        **_COMMON,
        "tags": ["descriptor-load-static", "descriptor-store-static", "program-id-1d"],
        "summary": (
            "Stage 2/5 of `torch.nn.functional.silu.2_spyre`: `aten.neg` "
    ...
    "exp": {
        **_COMMON,
        ...
        "summary": (
            "Stage 3/5 of `torch.nn.functional.silu.2_spyre`: in-place "
    ...
    "add_one": {
        **_COMMON,
        "tags": ["descriptor-load-static", "descriptor-store-static", "program-id-1d", "in-place"],
        "summary": (
            "Stage 4/5 of `torch.nn.functional.silu.2_spyre`: in-place "
    ...
    "div": {
        **_COMMON,
        "tags": ["descriptor-load-static", "descriptor-store-static", "program-id-1d", "in-place"],
        "summary": (
            "Stage 5/5 of `torch.nn.functional.silu.2_spyre`: in-place "
