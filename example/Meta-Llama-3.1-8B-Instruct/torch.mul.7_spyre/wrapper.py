...
the exact source path). See ``torch.mul.6_spyre/wrapper.py`` for the full
...
DISTRIBUTION_LOOP = False  # see torch.mul.6_spyre/wrapper.py
...
def mul(
    ...
    `[12, 1, 2, 8, 64]` (head axis moved front-to-back). See
    `torch.mul.6_spyre/wrapper.py` for the full derivation."""
...
def make_inputs(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    ...
    ``torch.mul.6_spyre/wrapper.py`` for the full rationale).
    """
    del xnumel, XBLOCK
...
def run(inputs: dict) -> np.ndarray:
    """NumPy oracle: identical derivation to ``torch.mul.6_spyre/wrapper.py``.
    """
