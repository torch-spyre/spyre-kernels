...
the exact source path). See ``torch.mul.12_spyre/meta.py`` for the full
...
def make_inputs(xnumel: int, XBLOCK: int, **_unused) -> dict:
    ...
    ``torch.mul.12_spyre/meta.py`` for the full rationale).
    """
    del xnumel, XBLOCK
...
def run(inputs: dict) -> np.ndarray:
    """NumPy oracle: identical derivation to ``torch.mul.12_spyre/meta.py``.
    """
...
VARIANTS = {
    "default": {
        ...
        "grid":       [32],
        "distribution_loop": False,  # see torch.mul.12_spyre/meta.py
        "reference":  run,
        "inputs":     make_inputs,
        "output_key": "out_ptr0",
    },
