"""``torch.cat.3_spyre`` kernel extracted from a torch-spyre Inductor trace.
...
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.cat.3`` op
(``aten.cat.default([x, y], dim=-1)`` on two logical ``f16[1, 8, 12, 64]``
tensors, producing a logical ``f16[1, 8, 12, 128]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.cat.3_spyre/
torch_compile_debug/run_2026_07_14_12_35_31_090085-pid_262537/
...
Unlike ``torch.cat.1_spyre``/``torch.cat.2_spyre``, the grid here (24
programs) is a genuinely 2D partition of the (8, 12) outer dims flattened
onto a single ``tl.program_id(0)`` axis: ``c0 = (pid // 12) * 4`` tiles the
outer (size-8) dim in 2 steps of 4 rows each (``block_shape[0] == 4``),
while ``c1 = pid % 12`` walks the middle (size-12) dim one row at a time —
``2 * 12 == 24 == grid``.
...
def triton_bundle_0_kernel_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    dim3 = 0
    dim4 = c2
    desc_0 = tl.make_tensor_descriptor(in_ptr0, shape=[8, 12, 1, 1, 64], strides=[768, 64, 64, 64, 1], block_shape=[4, 1, 1, 1, 64])
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[8, 12, 2, 1, 64], strides=[1536, 128, 64, 64, 1], block_shape=[4, 1, 2, 1, 64])
...
def triton_bundle_0_kernel_1(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[8, 12, 2, 1, 64], strides=[1536, 128, 64, 64, 1], block_shape=[4, 1, 2, 1, 64])
    tmp0 = desc_0.load([dim0, dim1, dim2, dim3, dim4])
    tmp1 = tl.reshape(tmp0, [4, 1, 1, 1, 64])
    tmp2 = tl.broadcast_to(tmp1, [4, 1, 2, 1, 64])
    desc_1.store([dim_1_0, dim_1_1, dim_1_2, dim_1_3, dim_1_4], tmp2)

@triton.jit
def triton_bundle_0(in_ptr0, in_ptr1, out_ptr0):
    triton_bundle_0_kernel_0(in_ptr0, out_ptr0, 6144, 256)
    triton_bundle_0_kernel_1(in_ptr1, out_ptr0, 6144, 256)
