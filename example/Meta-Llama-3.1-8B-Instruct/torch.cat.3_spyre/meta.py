...
- ``assert_size_stride(arg0_1, (1, 8, 12, 64), (6144, 768, 64, 1))`` / same
...
- ``triton_bundle_0.run(arg0_1, arg1_1, buf0, stream=stream0)`` — the entry
  ...
  by the two helpers (``6144``/``256``) are baked in as literals at the
  call sites inside ``triton_bundle_0`` itself.
- ``triton_meta={..., 'spyre_grid': (24,)}`` — 24-program grid: a 2D
  partition of the ``(8, 12)`` outer dims flattened onto
  ``tl.program_id(0)`` (``c0 = (pid // 12) * 4`` tiles the size-8 dim in 2
  steps of 4 rows; ``c1 = pid % 12`` walks the size-12 dim one row at a
  time; ``2 * 12 == 24``).

Both helpers hardcode the device shapes ``[8, 12, 1, 1, 64]`` (inputs) and
``[8, 12, 2, 1, 64]`` (output) into their tensor descriptors, so the
...
_IN_SHAPE = (8, 12, 1, 1, 64)
_OUT_SHAPE = (8, 12, 2, 1, 64)
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "Instruct's traced `torch.cat.3` op: concatenates two logical "
            "`[1, 8, 12, 64]` f16 tensors along the last dim, with a 2D "
            "outer-dim partition flattened onto one program-id axis."
        ),
        "doc": (
            "`aten.cat.default([x, y], dim=-1)` on two logical "
            "`[1, 8, 12, 64]` f16 tensors, producing `[1, 8, 12, 128]`. On "
            "the Spyre device layout each input's innermost 64-wide dim "
            "stays a single stick (`[8, 12, 1, 1, 64]`); the output pads "
            "the 128-wide concatenated dim out to two 64-wide sticks "
            "(`[8, 12, 2, 1, 64]`). The traced entry `triton_bundle_0` is "
            "a thin dispatcher that calls two `noinline=True` helpers in "
            "sequence: `triton_bundle_0_kernel_0` loads a 4-row tile of "
            "`in_ptr0` (shape `[4,1,1,1,64]`) and stores it directly into "
            "stick 0 of the output's doubled axis (axis 2) with no "
            "reshape/broadcast, while `triton_bundle_0_kernel_1` "
            "reshapes+broadcasts the matching tile of `in_ptr1` to "
            "`[4,1,2,1,64]` before storing into stick 1 — see the "
            "`disabled` note below, this asymmetry is a genuine bug in the "
            "traced source, not a mistake in this extraction. Unlike "
            "`torch.cat.1_spyre`/"
            "`torch.cat.2_spyre`, the grid here (24 programs) is a "
            "genuinely 2D partition of the `(8, 12)` outer dims flattened "
            "onto a single `tl.program_id(0)` axis: `c0 = (pid // 12) * 4` "
            "tiles the size-8 dim in 2 steps of 4 rows each "
            "(`block_shape[0] == 4`), while `c1 = pid % 12` walks the "
            "size-12 dim one row at a time (`2 * 12 == 24 == grid`). "
            "`xnumel`/`XBLOCK`/`xindex`/`xmask` inside the helpers are "
            "boilerplate inherited from Inductor's pointwise codegen: "
            "`xnumel` is immediately overwritten with the literal `6144` "
        ...
        "params":     {},
        "grid":       [24],
        # kernel_0 stores its loaded value (shape [4,1,1,1,64], from
        # desc_0's block_shape) directly into desc_1, whose block_shape is
        # [4,1,2,1,64] (the doubled concat axis) -- with no reshape/
        ...
        "disabled": {
            "reason": (
                "kernel_0's desc_1.store() passes a [4,1,1,1,64] value "
                "into a descriptor with block_shape [4,1,2,1,64] with no "
