"""``torch.mul.5_spyre`` kernel extracted from a torch-spyre Inductor trace.

The ``@triton.jit`` function body (from the ``async_compile.triton(...)``
source string) verbatim, from torch-spyre's Inductor output for
Meta-Llama-3.1-8B-Instruct's traced ``torch.mul.5`` op
(``aten.mul.Tensor(x, y)`` broadcasting a logical ``f16[1, 1, 12, 128]``
tensor ``y`` over the head dim of a logical ``f16[1, 32, 12, 128]`` tensor
``x``; structurally identical to ``torch.mul.4_spyre`` — same shapes,
strides and kernel body, just a distinct traced call site); see
``torch-spyre/test_results_triton_20260714_120532/torch.mul.5_spyre/
torch_compile_debug/run_2026_07_14_12_31_40_647161-pid_261129/
...
def triton_unk_fused_mul_0(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    dim3 = 0
    dim4 = c2 % 64
    desc_0 = tl.make_tensor_descriptor(in_ptr0, shape=[32, 12, 2, 1, 64], strides=[1536, 128, 64, 64, 1], block_shape=[1, 12, 2, 1, 64])
    # Logical layouts -> Device layouts
    dim_1_0 = 0
    dim_1_1 = c1
    dim_1_2 = c2 // 64
    dim_1_3 = 0
    dim_1_4 = c2 % 64
    desc_1 = tl.make_tensor_descriptor(in_ptr1, shape=[1, 12, 2, 1, 64], strides=[1536, 128, 64, 64, 1], block_shape=[1, 12, 2, 1, 64])
    # Logical layouts -> Device layouts
    dim_2_0 = c1
    dim_2_1 = 0
    dim_2_2 = c2 // 64
    dim_2_3 = c0
    dim_2_4 = c2 % 64
    desc_2 = tl.make_tensor_descriptor(out_ptr0, shape=[12, 1, 2, 32, 64], strides=[4096, 4096, 2048, 64, 1], block_shape=[12, 1, 2, 1, 64])
    tmp0 = desc_0.load([dim0, dim1, dim2, dim3, dim4])
    tmp2 = desc_1.load([dim_1_0, dim_1_1, dim_1_2, dim_1_3, dim_1_4])
    tmp1 = tl.reshape(tmp0, [12, 1, 2, 1, 64])
    tmp3 = tl.reshape(tmp2, [12, 1, 2, 1, 64])
    tmp4 = tmp1 * tmp3
