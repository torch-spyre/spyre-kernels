"""``torch.nn.functional.linear.5_spyre`` kernel extracted from a
...
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.nn.functional.linear.5``
op (``aten.mm`` between a logical ``f16[1, 1, 4096]`` activation — a
single decode-step token, M=1 — and a logical ``f16[128256, 4096]``
weight, i.e. the LM head projection to the full 128256-token vocabulary,
`nn.functional.linear` without bias); see ``torch-spyre/test_results_triton
_20260714_120532/torch.nn.functional.linear.5_spyre/torch_compile_debug/
run_2026_07_14_12_50_21_764706-pid_267017/torchinductor/
...
``triton_bundle_0`` is the entry that calls both in sequence. Unlike
``.1_spyre``-``.4_spyre`` (M=12), ``triton_bundle_0_kernel_1`` here has
*no* y-tree at all (``ynumel``/``YBLOCK`` are absent from its signature) —
Inductor's codegen drops the degenerate M=1 axis entirely, so this kernel
only tiles over x (N) and reduces over r0 (K).
