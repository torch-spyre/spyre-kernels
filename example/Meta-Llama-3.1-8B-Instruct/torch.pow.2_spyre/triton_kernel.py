"""``torch.pow.2_spyre`` kernel extracted from a torch-spyre Inductor trace.
...
Meta-Llama-3.1-8B-Instruct's traced ``torch.pow.2`` op
(``aten.pow.Tensor_Scalar(arg0_1, 2)`` — unary, integer exponent 2 — on a
logical ``f16[1, 1, 4096]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.pow.2_spyre/
torch_compile_debug/run_2026_07_14_13_00_41_054391-pid_269037/
...
def triton_unk_fused_pow_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[1, 64, 1, 64], strides=[4096, 64, 64, 1], block_shape=[1, 2, 1, 64])
