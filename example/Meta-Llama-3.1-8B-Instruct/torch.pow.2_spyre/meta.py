...
- ``triton_unk_fused_pow_0.run(arg0_1, buf0, 4096, stream=stream0)`` — the
  kernel is launched with ``xnumel=4096``.
...
Like ``torch.pow.1_spyre``, the traced graph fragment records
`aten.pow.Tensor_Scalar(arg0_1, 2)` but Inductor's codegen lowers the
integer exponent 2 to a repeated multiplication rather than a literal `**`
— the kernel body computes `tmp1 = tmp0 * tmp0` (see ``kernel.py``). The
oracle below mirrors that lowering exactly.

The kernel itself hardcodes shape ``[1, 64, 1, 64]`` into both tensor
...
def make_inputs(xnumel: int, XBLOCK: int, **_unused) -> dict:
    ...
    descriptors' hardcoded shape ``[1, 64, 1, 64]`` (see ``kernel.py``).
    """
    del xnumel, XBLOCK
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "Meta-Llama-3.1-8B-Instruct's traced `torch.pow.2` op, one "
            "program per `[1, 2, 1, 64]` block."
        ),
        "doc": (
            "Squares every element of a logical `[1, 1, 4096]` f16 "
            ...
            "the literal `4096` and `xmask` is a trivial all-`True` mask "
