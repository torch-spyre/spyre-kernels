"""``torch.nn.functional.linear.3_spyre`` kernel extracted from a
...
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.nn.functional.linear.3``
op (``aten.mm`` between a logical ``f16[1, 12, 4096]`` activation and a
logical ``f16[14336, 4096]`` weight, i.e. `nn.functional.linear` without
bias, out_features=14336 — the MLP up-projection width); see
``torch-spyre/test_results_triton_20260714_120532/torch.nn.functional.linear.3_spyre/
torch_compile_debug/run_2026_07_14_12_46_09_900218-pid_265815/
...
``triton_bundle_0`` is the entry that calls both in sequence.
...
def triton_bundle_0_kernel_1(in_ptr0, in_ptr1, out_ptr0, ynumel, xnumel, r0_numel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    dim_2_2 = c1 // 64
    dim_2_3 = c1 % 64
    desc_2 = tl.make_tensor_descriptor(out_ptr0, shape=[12, 1, 224, 64], strides=[14336, 64, 64, 1], block_shape=[12, 1, 7, 64])
    ...
    tmp9 = tl.reshape(tmp8, [12, 1, 7, 64])
...
def triton_bundle_0(in_ptr0, in_ptr1, out_ptr0, out_ptr1):
    ...
    triton_bundle_0_kernel_1(in_ptr1, out_ptr0, out_ptr1, 12, 14336, 4096, 12, 448)
