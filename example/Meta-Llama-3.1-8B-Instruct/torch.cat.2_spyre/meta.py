...
- ``assert_size_stride(arg0_1, (1, 32, 12, 64), (24576, 768, 64, 1))`` /
...
- ``buf0 = spyre_empty_with_layout((1, 32, 12, 128), (49152, 1536, 128, 1),
  ...
  — logical output ``f16[1, 32, 12, 128]``.
- ``triton_bundle_0.run(arg0_1, arg1_1, buf0, stream=stream0)`` — the entry
  ...
  by the two helpers (``24576``/``768``) are baked in as literals at the
  call sites inside ``triton_bundle_0`` itself.
- ``triton_meta={..., 'spyre_grid': (32,)}`` — 32-program grid, one core per
  ``dim0`` row of the device-layout shape ``[32, 12, 1, 1, 64]`` /
  ``[32, 12, 2, 1, 64]``.

Both helpers hardcode the device shapes ``[32, 12, 1, 1, 64]`` (inputs) and
``[32, 12, 2, 1, 64]`` (output) into their tensor descriptors, so the
...
_IN_SHAPE = (32, 12, 1, 1, 64)
_OUT_SHAPE = (32, 12, 2, 1, 64)
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "Instruct's traced `torch.cat.2` op: concatenates two logical "
            "`[1, 32, 12, 64]` f16 tensors along the last dim, one program "
            "per `dim0` row."
        ),
        "doc": (
            "`aten.cat.default([x, y], dim=-1)` on two logical "
            "`[1, 32, 12, 64]` f16 tensors, producing `[1, 32, 12, 128]`. On "
            "the Spyre device layout each input's innermost 64-wide dim "
            "stays a single stick, with a full `dim0`-row block spanning "
            "the whole middle (12) dim in one shot "
            "(`block_shape=[1, 12, 1, 1, 64]`); the output pads the "
            "128-wide concatenated dim out to two 64-wide sticks "
            "(`[32, 12, 2, 1, 64]`). The traced entry `triton_bundle_0` is "
            "a thin dispatcher that calls two `noinline=True` helpers in "
            "sequence: `triton_bundle_0_kernel_0` loads `in_ptr0` (shape "
            "`[1,12,1,1,64]`) and stores it directly into stick 0 of the "
            "output's doubled axis (axis 2) with no reshape/broadcast, "
            "while `triton_bundle_0_kernel_1` reshapes+broadcasts `in_ptr1` "
            "to `[1,12,2,1,64]` before storing into stick 1 — see the "
            "`disabled` note below, this asymmetry is a genuine bug in the "
            "traced source, not a mistake in this extraction. The grid is "
            "sized to 32 programs (one per `dim0` row), and each program "
            "reads `tl.program_id(0)` directly as its row index — no "
            ...
            "`xnumel` is immediately overwritten with the literal `24576` "
            "and `xmask` is a trivial all-`True` mask, never read — "
            "`XBLOCK` itself (=768, baked in as a call-site literal) is "
            "likewise dead and not power-of-2, same category as "
            "`torch.matmul.1_spyre`'s XBLOCK, though moot here since the "
            "store-shape bug below blocks compilation regardless."
        ),
        "kernel_fn":  kernel.triton_bundle_0,
        "constexpr":  [],
        "params":     {},
        "grid":       [32],
        # kernel_0 stores its loaded value (shape [1,12,1,1,64], from
        # desc_0's block_shape) directly into desc_1, whose block_shape is
        # [1,12,2,1,64] (the doubled concat axis) -- with no reshape/
        ...
        "disabled": {
            "reason": (
                "kernel_0's desc_1.store() passes a [1,12,1,1,64] value "
                "into a descriptor with block_shape [1,12,2,1,64] with no "
