...
- ``assert_size_stride(arg0_1, (1, 8, 12, 64), (6144, 768, 64, 1))`` —
  logical input, dtype ``torch.float16``.
- ``buf0 = spyre_empty_with_layout((1, 8, 12, 64), (6144, 768, 64, 1),
  torch.float16, SpyreTensorLayout(device_size=[8, 12, 1, 1, 64], ...))`` —
  logical output, same shape/dtype as the input.
- ``triton_unk_fused_neg_0.run(arg0_1, buf0, 6144, stream=stream0)`` — the
  kernel is launched with ``xnumel=6144``.
- ``triton_meta={..., 'spyre_grid': (24,)}`` — 24-program grid (grid !=
  xnumel): `dim0`/`dim1` come from `tl.program_id(0) // 12` and `% 12`
  respectively, each program covering a `[4, 1, 1, 1, 64]` block (256
  elements, matching `XBLOCK=256`).

The kernel itself hardcodes shape ``[8, 12, 1, 1, 64]`` into both tensor
...
def make_inputs(xnumel: int, XBLOCK: int, **_unused) -> dict:
    ...
    ``xnumel = 6144`` and never reads ``xindex``/``xmask`` for the actual
    descriptor indexing (`dim0`..`dim4` are derived straight from
    `tl.program_id(0)`), so the buffers are built directly at the
    descriptors' hardcoded shape ``[8, 12, 1, 1, 64]`` (see ``kernel.py``).
    """
    del xnumel, XBLOCK
    rng = np.random.default_rng(0)
    in_ptr0 = rng.standard_normal((8, 12, 1, 1, 64)).astype(np.float16)
    out_ptr0 = np.zeros((8, 12, 1, 1, 64), dtype=np.float16)
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "`torch.neg.2` op, one program per `[4, 1, 1, 1, 64]` block."
        ),
        "doc": (
            "Negates every element of a logical `[1, 8, 12, 64]` f16 "
            "tensor. On the Spyre device layout the innermost dim pads to a "
            "64-element f16 stick, giving a physical shape "
            "`[8, 12, 1, 1, 64]`; the grid is sized to 24 programs, each "
            "covering a `[4, 1, 1, 1, 64]` block (256 elements, matching "
            "`XBLOCK=256`) — `dim0`/`dim1` come from "
            "`tl.program_id(0) // 12` and `% 12`, no "
            ...
            "literal `6144`; `xmask = xindex < xnumel` is a real bounds "
            "check but the descriptor indexing never reads `xindex`/`xmask` "
            "either way — confirmed dead by the traced `.ttir`."
        ),
        "kernel_fn":  kernel.triton_unk_fused_neg_0,
        "constexpr":  ["XBLOCK"],
        "params":     {"xnumel": [6144], "XBLOCK": [256]},
        "grid":       [24],
