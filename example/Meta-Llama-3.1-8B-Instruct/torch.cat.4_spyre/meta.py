...
- ``assert_size_stride(arg0_1, (1, 1, 64), (64, 64, 1))`` / same for
  ``arg1_1`` — two logical inputs, both dtype ``torch.float16``.
- ``buf0 = spyre_empty_with_layout((1, 1, 128), (128, 128, 1),
  torch.float16, SpyreTensorLayout(device_size=[1, 2, 1, 64], ...))`` —
  logical output ``f16[1, 1, 128]``.
- ``triton_bundle_0.run(arg0_1, arg1_1, buf0, stream=stream0)`` — the entry
  ...
  by the two helpers (``64``/``64``) are baked in as literals at the call
  sites inside ``triton_bundle_0`` itself.
- ``triton_meta={..., 'spyre_grid': (1,)}`` — a trivial single-program
  grid; the whole (degenerate, size-1) outer shape is handled by one
  program, so the "Triton -> Logical layouts" section hardcodes ``c0 = 0``
  rather than reading ``tl.program_id(0)``.

Both helpers hardcode the device shapes ``[1, 1, 1, 64]`` (inputs) and
``[1, 2, 1, 64]`` (output) into their tensor descriptors, so the pointer
args here are built directly at those shapes. ``kernel_0`` writes
``in_ptr0`` into index 0 of the doubled axis (axis 1); ``kernel_1`` writes
...
VARIANTS = {
    "default": {
        "tags": ["descriptor-load-static", "descriptor-store-static", "single-program", "bundled-multi-kernel"],
        "summary": (
            "Bundled two-kernel `aten.cat` implementing Meta-Llama-3.1-8B-"
            "Instruct's traced `torch.cat.4` op: concatenates two logical "
            "`[1, 1, 64]` f16 tensors along the last dim, in a single "
            "1-program grid."
        ),
        "doc": (
            "`aten.cat.default([x, y], dim=-1)` on two logical `[1, 1, 64]` "
            "f16 tensors, producing `[1, 1, 128]`. On the Spyre device "
            "layout the innermost 64-wide dim of each input stays a single "
            "stick (`[1, 1, 1, 64]`), and the output pads the 128-wide "
            "concatenated dim out to two 64-wide sticks (`[1, 2, 1, 64]`). "
            "The traced entry `triton_bundle_0` is a thin dispatcher that "
            "calls two `noinline=True` helpers in sequence: "
            "`triton_bundle_0_kernel_0` loads `in_ptr0` (shape "
            "`[1,1,1,64]`) and stores it directly into stick 0 of the "
            "output's doubled axis with no reshape/broadcast, while "
            "`triton_bundle_0_kernel_1` reshapes+broadcasts `in_ptr1` to "
            "`[1,2,1,64]` before storing into stick 1 — see the "
            "`disabled` note below, this asymmetry is a genuine bug in the "
            "traced source, not a mistake in this extraction. This is the "
            "degenerate single-program case: the grid is exactly 1, and "
            "the 'Triton -> Logical layouts' section hardcodes `c0 = 0` "
            "as a constant rather than reading `tl.program_id(0)` (though "
            "`tl.program_id(0)` is still called once, purely for the dead "
            "`xoffset`/`xindex`/`xmask` boilerplate). "
            "`xnumel`/`XBLOCK`/`xindex`/`xmask` inside the helpers are "
            "boilerplate inherited from Inductor's pointwise codegen: "
            "`xnumel` is immediately overwritten with the literal `64` and "
            "`xindex`/`xmask` are never read."
        ),
        "kernel_fn":  kernel.triton_bundle_0,
        "constexpr":  [],
        "params":     {},
        "grid":       [1],
        # kernel_0 stores its loaded value (shape [1,1,1,64], from
        # desc_0's block_shape) directly into desc_1, whose block_shape is
        # [1,2,1,64] (the doubled concat axis) -- with no reshape/
        ...
        "disabled": {
            "reason": (
                "kernel_0's desc_1.store() passes a [1,1,1,64] value into "
                "a descriptor with block_shape [1,2,1,64] with no reshape/"
                "broadcast (kernel_1's store does reshape+broadcast) -- a "
                "shape mismatch present verbatim in the traced source, "
                "rejected by Triton's frontend validate_store_like check "
                "before TTIR construction. Reproduces identically across "
                "all torch.cat.*_spyre examples."
            ),
        },
        "reference":  run,
        "inputs":     make_inputs,
