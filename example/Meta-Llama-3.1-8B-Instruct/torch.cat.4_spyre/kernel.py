"""``torch.cat.4_spyre`` kernel extracted from a torch-spyre Inductor trace.

The three ``@triton.jit`` functions verbatim (lines 61-113 of the
``async_compile.triton(...)`` source string), from torch-spyre's Inductor
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.cat.4`` op
(``aten.cat.default([x, y], dim=-1)`` on two logical ``f16[1, 1, 64]``
tensors, producing a logical ``f16[1, 1, 128]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.cat.4_spyre/
torch_compile_debug/run_2026_07_14_12_55_54_063110-pid_268078/
...
- ``triton_bundle_0_kernel_0`` — loads a tile of ``in_ptr0`` (the first cat
  ...
  axis 1) of the output's device layout.
- ``triton_bundle_0_kernel_1`` — loads a tile of ``in_ptr1`` (the second cat
  ...
  shape, and stores it into the back slot (index 1 of the doubled axis 1)
...
Trivial single-program case: the grid here is just 1 program, and the
"Triton -> Logical layouts" section hardcodes ``c0 = 0`` (a constant, not
derived from ``tl.program_id``), even though ``tl.program_id(0)`` is still
read once (for the dead ``xoffset``/``xindex``/``xmask`` boilerplate).
...
def triton_bundle_0_kernel_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[1, 2, 1, 64], strides=[128, 64, 64, 1], block_shape=[1, 2, 1, 64])
...
def triton_bundle_0(in_ptr0, in_ptr1, out_ptr0):
    triton_bundle_0_kernel_0(in_ptr0, out_ptr0, 64, 64)
    triton_bundle_0_kernel_1(in_ptr1, out_ptr0, 64, 64)
