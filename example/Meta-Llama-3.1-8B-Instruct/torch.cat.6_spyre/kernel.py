"""``torch.cat.6_spyre`` kernel extracted from a torch-spyre Inductor trace.
...
output for Meta-Llama-3.1-8B-Instruct's traced ``torch.cat.6`` op
(``aten.cat.default([x, y], dim=-1)`` on two logical ``f16[1, 8, 1, 64]``
tensors, producing a logical ``f16[1, 8, 1, 128]`` tensor); see
``torch-spyre/test_results_triton_20260714_120532/torch.cat.6_spyre/
torch_compile_debug/run_2026_07_14_13_21_31_645676-pid_273392/
...
One program per row of the outermost (size-8) dim: ``c0 =
tl.program_id(0)`` indexes ``dim0`` directly, 8 programs total.
...
def triton_bundle_0_kernel_0(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    dim3 = 0
    dim4 = c1
    desc_0 = tl.make_tensor_descriptor(in_ptr0, shape=[8, 1, 1, 1, 64], strides=[64, 64, 64, 64, 1], block_shape=[1, 1, 1, 1, 64])
    desc_1 = tl.make_tensor_descriptor(out_ptr0, shape=[8, 1, 2, 1, 64], strides=[128, 128, 64, 64, 1], block_shape=[1, 1, 2, 1, 64])
...
def triton_bundle_0(in_ptr0, in_ptr1, out_ptr0):
    triton_bundle_0_kernel_0(in_ptr0, out_ptr0, 512, 64)
    triton_bundle_0_kernel_1(in_ptr1, out_ptr0, 512, 64)
