...
- ``assert_size_stride(arg0_1, (1, 8, 1, 64), (512, 64, 64, 1))`` / same
...
- ``triton_bundle_0.run(arg0_1, arg1_1, buf0, stream=stream0)`` — the entry
  ...
  by the two helpers (``512``/``64``) are baked in as literals at the call
  sites inside ``triton_bundle_0`` itself.
- ``triton_meta={..., 'spyre_grid': (8,)}`` — 8-program grid, one core per
  row of the device-layout shape ``[8, 1, 1, 1, 64]`` / ``[8, 1, 2, 1,
  64]``.

Both helpers hardcode the device shapes ``[8, 1, 1, 1, 64]`` (inputs) and
``[8, 1, 2, 1, 64]`` (output) into their tensor descriptors, so the
...
_IN_SHAPE = (8, 1, 1, 1, 64)
_OUT_SHAPE = (8, 1, 2, 1, 64)
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "Instruct's traced `torch.cat.6` op: concatenates two logical "
            "`[1, 8, 1, 64]` f16 tensors along the last dim, one program "
            "per row."
        ),
        "doc": (
            "`aten.cat.default([x, y], dim=-1)` on two logical "
            "`[1, 8, 1, 64]` f16 tensors, producing `[1, 8, 1, 128]`. On "
            "the Spyre device layout each input's innermost 64-wide dim "
            "stays a single stick (`[8, 1, 1, 1, 64]`); the output pads "
            "the 128-wide concatenated dim out to two 64-wide sticks "
            "(`[8, 1, 2, 1, 64]`). The traced entry `triton_bundle_0` is "
            ...
            "The grid is sized to exactly 8 programs (one per `dim0` row), "
