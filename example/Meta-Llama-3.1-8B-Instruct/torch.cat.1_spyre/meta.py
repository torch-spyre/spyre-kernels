...
- ``assert_size_stride(arg0_1, (1, 12, 64), (768, 64, 1))`` / same for
  ``arg1_1`` — two logical inputs, both dtype ``torch.float16``.
- ``buf0 = spyre_empty_with_layout((1, 12, 128), (1536, 128, 1),
  torch.float16, SpyreTensorLayout(device_size=[12, 2, 1, 64], ...))`` —
  logical output ``f16[1, 12, 128]``.
- ``triton_bundle_0.run(arg0_1, arg1_1, buf0, stream=stream0)`` — the entry
  ...
  by the two helpers (``768``/``64``) are baked in as literals at the call
  sites inside ``triton_bundle_0`` itself, never exposed at the entry's own
  signature.
- ``triton_meta={..., 'spyre_grid': (12,)}`` — 12-program grid, one core per
  row of the device-layout shape ``[12, 1, 1, 64]`` / ``[12, 2, 1, 64]``.

Both helpers hardcode the device shapes ``[12, 1, 1, 64]`` (inputs) and
``[12, 2, 1, 64]`` (output) into their tensor descriptors, so the pointer
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "Instruct's traced `torch.cat.1` op: concatenates two logical "
            "`[1, 12, 64]` f16 tensors along the last dim, one program per "
            "row."
        ),
        "doc": (
            "`aten.cat.default([x, y], dim=-1)` on two logical `[1, 12, 64]` "
            "f16 tensors, producing `[1, 12, 128]`. On the Spyre device "
            "layout the innermost 64-wide dim of each input stays a single "
            "stick (`[12, 1, 1, 64]`), and the output pads the 128-wide "
            "concatenated dim out to two 64-wide sticks (`[12, 2, 1, 64]`). "
            ...
            "output's doubled axis, while `triton_bundle_0_kernel_1` "
            "reshapes+broadcasts `in_ptr1` to `[1,2,1,64]` before storing "
            "into stick 1 — see the `disabled` note below, this asymmetry "
            "is a genuine bug in the traced source, not a mistake in this "
            "extraction. The grid is sized to exactly 12 programs (one per "
            "row), and each program reads `tl.program_id(0)` directly as "
            "its row index — no `num_programs`/`cdiv` distribution loop in "
            "the source. `xnumel`/`XBLOCK`/`xindex`/`xmask` inside the "
            "helpers are boilerplate inherited from Inductor's pointwise "
            "codegen: `xnumel` is immediately overwritten with the literal "
            "`768` and `xindex`/`xmask` are never read."
        ),
