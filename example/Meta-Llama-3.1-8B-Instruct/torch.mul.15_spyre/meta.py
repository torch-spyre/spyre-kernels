...
and kernel body, just a distinct traced call site (see ``kernel.py`` for
the exact source path). See ``torch.mul.14_spyre/meta.py`` for the full
derivation of the reference oracle and device-layout reasoning; this file
duplicates it verbatim.

Caller-side settings recap:

- logical ``x``: ``[1, 8, 1, 128]`` f16; logical ``y``: ``[1, 1, 1,
  128]`` f16, broadcast over ``x``'s head dim; logical output: same shape
  as ``x``.
- device shapes: ``in_ptr0`` ``[8, 1, 2, 1, 64]``, ``in_ptr1`` ``[1, 1,
  2, 1, 64]``, ``out_ptr0`` ``[1, 1, 2, 8, 64]``.
...
def make_inputs(xnumel: int, XBLOCK: int, **_unused) -> dict:
    ...
    ``torch.mul.14_spyre/meta.py`` for the full rationale).
    """
    del xnumel, XBLOCK
...
def run(inputs: dict) -> np.ndarray:
    """NumPy oracle: identical derivation to ``torch.mul.14_spyre/meta.py``.
    """
...
VARIANTS = {
    "default": {
        ...
        "grid":       [16],
        "distribution_loop": False,  # see torch.mul.14_spyre/meta.py
        "reference":  run,
        "inputs":     make_inputs,
        "output_key": "out_ptr0",
    },
