...
K=14336, N=4096 (MLP down-projection, mirror of ``.3_spyre``'s
up-projection). ``spyre_grid: (32,)``.

Per-pointer device layout (plain C-contiguous reshapes, no permutation
at this level — see ``kernel.py``):

- ``in_ptr0``  (raw weight,     logical ``[4096, 14336]`` = N x K) -> device ``[224, 4096, 64]``
- ``in_ptr1``  (raw activation, logical ``[12, 14336]``   = M x K) -> device ``[12, 1, 224, 64]``
- ``out_ptr0`` (scratch)                                           -> device ``[64, 14336, 64]``
...
M, K, N = 12, 14336, 4096
WEIGHT_DEV_SHAPE = (224, 4096, 64)   # N*K = 58720256
ACT_DEV_SHAPE = (M, 1, 224, 64)      # M*K = 172032
SCRATCH_DEV_SHAPE = (64, 14336, 64)  # N*K = 58720256
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "traced `torch.nn.functional.linear.4` op: M=12, K=14336, "
            "N=4096 (MLP down-projection)."
        ),
        "doc": (
            "Same structure as `.1_spyre`. Like `.3_spyre`, kernel_0's own "
            "XBLOCK is non-power-of-2 (1835008 = 58720256 / 32), so the "
            "(dead) `xindex = xoffset + tl.arange(0, XBLOCK)[:]` line in "
            "kernel_0 — which runs *first* inside the entry — trips the "
            "power-of-2 gate before kernel_1 is even reached. Note "
            "kernel_1's r0_numel=14336/R0_BLOCK=16384 masking "
            "(`r0_mask = r0_index < r0_numel`) does not itself introduce a "
            "reduction loop (the tensor-descriptor block shapes are still "
            "sized to the exact K=14336), so it is irrelevant to whether "
            "this kernel compiles — the failure is purely kernel_0's "
            "XBLOCK."
        ),
        ...
        # 58720256, 1835008)`) -- same value/root cause as `.3_spyre`
        # (its M/K/N differ, but 58720256/32 collapses to the same
        # non-power-of-2 XBLOCK here). kernel_0 runs first inside the
        # entry, so this trips the frontend before kernel_1 is reached.
