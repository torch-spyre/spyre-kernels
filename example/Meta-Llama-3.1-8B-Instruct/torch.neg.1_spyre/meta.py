...
- ``buf0 = spyre_empty_with_layout((1, 32, 12, 64), (24576, 768, 64, 1),
  torch.float16, SpyreTensorLayout(device_size=[32, 12, 1, 1, 64], ...))`` —
  logical output, same shape/dtype as the input.
- ``triton_unk_fused_neg_0.run(arg0_1, buf0, 24576, stream=stream0)`` — the
  kernel is launched with ``xnumel=24576``.
- ``triton_meta={..., 'spyre_grid': (32,)}`` — 32-program grid (grid !=
  xnumel): each program handles one `dim0` row, a `[1, 12, 1, 1, 64]` block
  (768 elements, matching `XBLOCK=768`).

The kernel itself hardcodes shape ``[32, 12, 1, 1, 64]`` into both tensor
descriptors, so the pointer args here are built directly at that shape.

NOTE on ``XBLOCK``: the source's own ``XBLOCK`` is ``768`` (not a power of
...
``tl.arange(0, 768)`` at compile time with "arange's range must be a power
...
power of 2 (``1024``) purely to satisfy that frontend check — a reviewer
...
def make_inputs(xnumel: int, XBLOCK: int, **_unused) -> dict:
    ...
    ``xnumel = 24576`` and never reads ``xindex``/``xmask`` (``xmask`` is a
    trivial all-``True`` mask), so the buffers are built directly at the
    descriptors' hardcoded shape ``[32, 12, 1, 1, 64]`` (see ``kernel.py``).
    """
    del xnumel, XBLOCK
    rng = np.random.default_rng(0)
    in_ptr0 = rng.standard_normal((32, 12, 1, 1, 64)).astype(np.float16)
    out_ptr0 = np.zeros((32, 12, 1, 1, 64), dtype=np.float16)
...
VARIANTS = {
    "default": {
        ...
        "summary": (
            ...
            "`torch.neg.1` op, one program per `dim0` row."
        ),
        "doc": (
            "Negates every element of a logical `[1, 32, 12, 64]` f16 "
            "tensor. On the Spyre device layout the innermost dim pads to a "
            "64-element f16 stick, giving a physical shape "
            "`[32, 12, 1, 1, 64]`; the grid is sized to exactly 32 programs "
            "(one per `dim0` row of 768 elements, matching `XBLOCK=768`), "
            ...
            "overwritten with the literal `24576` and `xmask` is a trivial "
            "all-`True` mask (`tl.full([XBLOCK], True, tl.int1)`), never "
            "read for masking."
        ),
        "kernel_fn":  kernel.triton_unk_fused_neg_0,
        "constexpr":  ["XBLOCK"],
        # XBLOCK substituted from the source's 768 to the next power of 2
        # (1024) — see the "NOTE on XBLOCK" in the module docstring above:
        # XBLOCK only feeds dead xindex/xmask boilerplate, but Triton's
        # `tl.arange` requires a power-of-2 range at compile time.
        "params":     {"xnumel": [24576], "XBLOCK": [1024]},
        "grid":       [32],
