...
- ``assert_size_stride(arg0_1, (1, 12, 4096), (49152, 4096, 1))`` / same for
...
The kernel itself hardcodes shape ``[12, 64, 1, 64]`` into all three tensor
...
SUMMARY = (
    ...
    "traced `torch.add.4` op, one program per `[12, 2, 1, 64]` "
    "block."
)

DOC = (
    "Adds two logical `[1, 12, 4096]` f16 tensors elementwise. On "
...
# expressed via the real descriptor tiling instead of the dead
# xnumel/XBLOCK placeholders.
...
def make_inputs(xnumel: int = XNUMEL, XBLOCK: int = XBLOCK, **_unused) -> dict:
    ...
    in_ptr0 = rng0.standard_normal((12, 64, 1, 64)).astype(np.float16)
    in_ptr1 = rng1.standard_normal((12, 64, 1, 64)).astype(np.float16)
